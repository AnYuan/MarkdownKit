...
| 测试文件数 (`Tests/MarkdownKitTests/*.swift`) | 84 | 含基准/夹具/辅助文件 |
| 含 `test*` 方法的测试文件 | 76 | 静态扫描结果 |
| 静态扫描 `test*` 方法总数 | 806 | 受编译条件影响，可能高于可执行测试数 |
| 可发现测试数 (`swift test list`) | 701 | 当前平台可执行测试 |
...
| `ArithmeticListQuoteRoutingTests.swift` | 13 |
| `ArithmeticParagraphModelTests.swift` | 21 |
| `ArithmeticTextCalculatorTests.swift` | 36 |
| `ArithmeticTextMeasurerTests.swift` | 17 |
...
| `ConcurrencyStressTests.swift` | 5 |
...
| `InlineFormattingLayoutTests.swift` | 34 |
...
| `PreparedContentCacheTests.swift` | 42 |
