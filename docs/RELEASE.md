...
| Fast correctness | `bash scripts/verify_fast.sh` | 602 tests; 621 discoverable tests. |
| Documentation freshness | `bash scripts/check_doc_freshness.sh` | 621 discoverable tests. |
...
| iOS Simulator correctness | `bash scripts/verify_ios.sh` | Exactly 662 XCTest tests, then exactly one PASS marker from an app-hosted `MarkdownView` Mermaid fence using real WebKit. |
...
662-test contract passes, the script reuses the package build to assemble, sign, install, and
