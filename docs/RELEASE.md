...
| iOS Simulator correctness | `bash scripts/verify_ios.sh` | Exactly 575 XCTest tests, then exactly one PASS marker from an app-hosted `MarkdownView` Mermaid fence using real WebKit. |
...
575-test contract passes, the script reuses the package build to assemble, sign, install, and
