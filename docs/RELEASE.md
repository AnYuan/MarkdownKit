...
| Fast correctness | `bash scripts/verify_fast.sh` | 614 tests; 633 discoverable tests. |
| Documentation freshness | `bash scripts/check_doc_freshness.sh` | 633 discoverable tests. |
...
| iOS Simulator correctness | `bash scripts/verify_ios.sh` | Exactly 674 XCTest tests, then exactly one PASS marker from an app-hosted `MarkdownView` Mermaid fence using real WebKit. |
...
674-test contract passes, the script reuses the package build to assemble, sign, install, and
