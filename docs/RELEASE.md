...
| Fast correctness | `bash scripts/verify_fast.sh` | 682 tests; 701 discoverable tests. |
| Documentation freshness | `bash scripts/check_doc_freshness.sh` | 701 discoverable tests. |
...
| iOS Simulator correctness | `bash scripts/verify_ios.sh` | Exactly 746 XCTest tests, then exactly one PASS marker from an app-hosted `MarkdownView` Mermaid fence using real WebKit. |
...
746-test contract passes, the script reuses the package build to assemble, sign, install, and
