...
| Fast correctness | `bash scripts/verify_fast.sh` | 661 tests; 680 discoverable tests. |
| Documentation freshness | `bash scripts/check_doc_freshness.sh` | 680 discoverable tests. |
...
| iOS Simulator correctness | `bash scripts/verify_ios.sh` | Exactly 725 XCTest tests, then exactly one PASS marker from an app-hosted `MarkdownView` Mermaid fence using real WebKit. |
...
725-test contract passes, the script reuses the package build to assemble, sign, install, and
