...
| Fast correctness | `bash scripts/verify_fast.sh` | 512 tests; 530 discoverable tests. |
| Documentation freshness | `bash scripts/check_doc_freshness.sh` | 530 discoverable tests. |
...
| iOS Simulator correctness | `bash scripts/verify_ios.sh` | Exactly 563 XCTest tests, then exactly one PASS marker from an app-hosted `MarkdownView` Mermaid fence using real WebKit. |
...
563-test contract passes, the script reuses the package build to assemble, sign, install, and
