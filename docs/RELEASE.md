...
| Fast correctness | `bash scripts/verify_fast.sh` | 514 tests; 532 discoverable tests. |
| Documentation freshness | `bash scripts/check_doc_freshness.sh` | 532 discoverable tests. |
...
| iOS Simulator correctness | `bash scripts/verify_ios.sh` | Exactly 565 XCTest tests, then exactly one PASS marker from an app-hosted `MarkdownView` Mermaid fence using real WebKit. |
...
565-test contract passes, the script reuses the package build to assemble, sign, install, and
