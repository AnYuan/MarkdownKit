...
| Fast correctness | `bash scripts/verify_fast.sh` | 596 tests; 615 discoverable tests. |
| Documentation freshness | `bash scripts/check_doc_freshness.sh` | 615 discoverable tests. |
...
| iOS Simulator correctness | `bash scripts/verify_ios.sh` | Exactly 656 XCTest tests, then exactly one PASS marker from an app-hosted `MarkdownView` Mermaid fence using real WebKit. |
...
656-test contract passes, the script reuses the package build to assemble, sign, install, and
